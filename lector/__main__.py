...
class MainUI(QtWidgets.QMainWindow, mainwindow.Ui_MainWindow):
    def __init__(self):
        ...
        self.libraryToolBar.colorButton.triggered.connect(self.showDialog)
        ...
        file = os.listdir('resources/raw/BackgroundImage/')[0]
        if file != 'BI.png':
    ...
    def showDialog(self):
        mb = QtWidgets.QMessageBox()  # 创建对话框
        mb.setWindowTitle('设置背景')  # 设置标题
        mb.setIconPixmap(QtGui.QPixmap('resources/raw/ansy2.png').scaled(60,60))  # 设置对话框的图标
        mb.setWindowIcon(QtGui.QIcon('resources/raw/DarkIcons/color-picker.svg'))  # 设置对话框的图标

        mb.setText('请选择背景设置方式')  # 设置文本

        color_button = QtWidgets.QPushButton('颜色')  # 创建按钮
        mb.addButton(color_button, QtWidgets.QMessageBox.AcceptRole)  # 添加自定义按钮并指定种类
        image_button = QtWidgets.QPushButton('图片')
        mb.addButton(image_button, QtWidgets.QMessageBox.AcceptRole)
        cancel_button = QtWidgets.QPushButton('取消')
        mb.addButton(cancel_button, QtWidgets.QMessageBox.RejectRole)
        mb.exec_()  # 运行自定义对话框
        select = mb.clickedButton().text()
        if select == '颜色':
            self.get_color()
        if select == '图片':
            self.choose_bgimage()
